...
import Exclamation from './StylableExclamation';
...
@stylable(styles)
export default class InputArea extends WixComponent {

  static displayName = 'InputArea';
  ...
  render() {
    ...
    return (
      <div>
        <div style-state={classes} className={`inputAreaRoot theme-${theme}`}>
          <textarea
            rows={rows}
            maxLength={maxLength}
            ...
            readOnly={readOnly}
            />
          {theme === 'material' && <div className="bar"/>}
          {hasCounter && maxLength && <span className="counter">{this.state.counter}/{maxLength}</span>}
        </div>
        <div className="error">
